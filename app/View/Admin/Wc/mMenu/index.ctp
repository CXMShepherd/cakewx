...
<div style="margin-bottom:15px">
	<div class="btn-group">
		...
		<ul class="dropdown-menu">
			<li><?php echo $this->Html->link('微官网', "{$WC_URL}?_m=mSite&_a=add"); ?></li>
			<li><?php echo $this->Html->link('微店铺', "{$WC_URL}?_m=mStore&_a=add"); ?></li>
		</ul>
	</div>
</div>
<div class="a_types">
				...
				<tbody>
					<? if (is_array($data['datalist'])): ?>
						<? foreach ($data['datalist'] as $key => $vals): ?>
							<tr>
								<td>
									<?= $vals['WxDataMenu']['FName'] ?>
								</td>
								<td>
                                   <?= $vals['WxDataMenu']['C_FType'] ?>
                                </td>
								<td class="hidden-480">
									<p><?= $vals['WxDataMenu']['FCreatedate'] ?></p>
								</td>
								<td>
									<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">

										<button class="btn btn-xs btn-info" onclick="parent.location.href='<?php echo "{$WC_URL}?_a=edit&_id={$vals['WxDataMenu']['Id']}" ?>'">
											<i class="icon-edit bigger-120" title="编辑"></i>
										</button>

										<button class="btn btn-xs btn-danger bootbox-confirm" alt="<?php echo "{$WC_URL}?_a=del&_id={$vals['WxDataMenu']['Id']}" ?>">
