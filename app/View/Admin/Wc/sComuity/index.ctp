...
<div style="margin-bottom:15px">
	<div class="btn-group">
		...
		<ul class="dropdown-menu">
			<li><?php echo $this->Html->link('个人社区', "{$WC_URL}?_m=00&_a=add"); ?></li>
			<li><?php echo $this->Html->link('物业公司', "{$WC_URL}?_m=1&_a=add"); ?></li>
		</ul>
	</div>
</div>
<div class="a_types">
				...
				<tbody>
					<? if (is_array($data['datalist'])): ?>
						<? foreach ($data['datalist'] as $key => $vals): ?>
							<tr>
								<td>
									<?= $vals['WxDataPlace']['FName'] ?>
								</td>
								<td>
                                   <?= $vals['WxDataPlace']['C_FType'] ?>
                                </td>
								<td class="hidden-480">
									<p><?= $vals['WxDataPlace']['FCreatedate'] ?></p>
								</td>
								<td>
									<a href="<?php echo "{$WC_URL}?_m=store&_id={$vals['WxDataPlace']['Id']}" ?>" class="btn btn-success btn-xs">
										<i class="icon-shopping-cart"></i>
										商家管理
									</a>
									<a href="<?php echo "{$WC_URL}?_m=article&_id={$vals['WxDataPlace']['Id']}" ?>" class="btn btn-warning btn-xs">
										<i class="icon-shopping-cart"></i>
										文章管理
									</a>
								</td>
								<td>
									<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
										...
										<button class="btn btn-xs btn-info" onclick="parent.location.href='<?php echo "{$WC_URL}?_a=edit&_id={$vals['WxDataPlace']['Id']}" ?>'">
											<i class="icon-edit bigger-120" title="编辑"></i>
										</button>
										<button class="btn btn-xs btn-danger bootbox-confirm" alt="<?php echo "{$WC_URL}?_a=del&_id={$vals['WxDataPlace']['Id']}" ?>">
											<i class="icon-trash bigger-120" title="删除"></i>
										</button>
										<a class="btn btn-xs btn-warning" target="_blank" href="<?php echo "{$vals['WxDataPlace']['fontendUrl']}"; ?>">
