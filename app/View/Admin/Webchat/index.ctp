...
?>
<button class="btn btn-sm btn-primary"  onclick="location.href='<?= Router::url(array('controller' => "admin", 'action' => "webchat", "add")) ?>'">
...
<div class="alert" style="margin-top:15px">
	...
	<strong>温馨提示：</strong>
		您还有<?= $data['leavecount'] ?>个移动应用配额，请珍惜使用名额！
	<br>
</div>
					...
					<? if (is_array($data['datalist'])): ?>
						<? foreach ($data['datalist'] as $key => $vals): ?>
							<tr>
								...
								<td class="wc_actions">
                                    <button class="e_btn1" onclick="parent.location.href='<?= Router::url(array('controller' => "admin", 'action' => "webchat", "edit", $vals['WxWebchat']['Id'])) ?>'">
                                        <i class="icon-edit bigger-120"></i>编辑
                                    </button>

                                    <button class="e_btn1 mtm bootbox-confirm" alt="<?= Router::url(array('controller' => "admin", 'action' => "webchat", "del", $vals['WxWebchat']['Id'])) ?>">
